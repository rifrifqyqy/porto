    ...
    <>
      ...
      <nav className="flex sticky top-0 z-50 bg-white justify-between px-12 max-md:px-4 py-4 W-full border-b-[1px] items-center border-slate-800 ">
        ...
        <AnimatePresence>
          <motion.button
            initial={{ scale: 1 }}
            whileTap={{ scale: 0, rotate: "180deg" }}
            exit={{ scale: 1 }}
            onMouseDown={handleNavMobile}
            onTouchStart={handleNavMobile}
            className="hidden bg-[#F8CC76] max-md:block border-[2px] border-gray-800"
          >
