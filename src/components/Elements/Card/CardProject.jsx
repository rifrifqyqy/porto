  ...
  return (
    <>
      <main className={twMerge("p-4 max-md:p-2 bg-white border-[1px] flex flex-col border-gray-800 max-w-min gap-4 max-md:gap-2 hover:bg-sky-700 justify-between group", className, transitionStyle)}>
        ...
        <MainButton className={twMerge(" bg-sky-700 text-center font-statliches text-[20px] max-md:text-[14px] tracking-wider group-hover:bg-white group-hover:text-sky-700", buttonStyle, transitionStyle)}>Lihat Detail</MainButton>
      </main>
    </>
  );
}
...
function CardHeader({ children, styleHeader, labelLink, labelTag }) {
  return (
    <div className="flex justify-between items-center">
      <div className={twMerge("flex gap-2 bg-sky-100 border-[1px] border-black items-center group-hover:bg-white h-[38px] max-md:h-[28px] w-[42px] max-md:w-[36px] group-hover:w-fit group-hover:pr-4 max-md:group-hover:pr-1 cursor-pointer", transitionStyle)}>
        <div className="flex w-fit gap-2 items-center pl-1 h-full">
          ...
          <Link to={labelLink} className={twMerge("text-[16px] max-md:text-[12px] opacity-0 group-hover:opacity-100 text-sky-700 font-semibold hidden group-hover:block", transitionStyle)}>{labelTag}</Link>
        </div>
      </div>
